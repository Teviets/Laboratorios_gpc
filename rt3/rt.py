...
class Raytracer(object):
    ...
    def rtRayColor(self, intercept, rayDirection, recursion=0):
        ...
        if material.matType == OPAQUE:
            ...
            reflect = fnp.VectorReflected(fnp.ScalarPerVector(rayDirection,-1), intercept.normal)
            ...
            for light in self.lights:
                if light.lightType != "Ambient":
                    lightDir = None
                    if light.lightType == "Directional":
                        lightDir = fnp.ScalarPerVector(light.direction,-1)
                    elif light.lightType == "Point":
                        lightDir = fnp.elementwise_subtract(light.point, intercept.point)
                        lightDir = fnp.normalVector(lightDir)
            ...
            outside = fnp.dot_product(rayDirection, intercept.normal) < 0
            bias = fnp.ScalarPerVector(intercept.normal, 0.001)

            reflect = fnp.VectorReflected(fnp.ScalarPerVector(rayDirection,-1), intercept.normal)
            reflectOrigin = fnp.vector_addition(reflect, bias) if outside else fnp.elementwise_subtract(reflect, bias)
            ...
            if not fnp.InternalReflection(intercept.normal, rayDirection, 1.0, material.ior):
                refract = fnp.refractVector(intercept.normal, rayDirection, 1, material.ior)
                refractOrigin = fnp.elementwise_subtract(intercept.point, bias) if outside else fnp.vector_addition(intercept.point, bias)
                ...
                kr, kt = fnp.fresnel(intercept.normal, rayDirection, 1, material.ior)
